...
class YOLO_detector(object):
    ...
    def setup_meta_ops(self):
        ...
        utility = min(self.gpu_utility, 1.0)  # GPU利用率
        if utility > 0.0:
    ...
    def resize_input(self, im):
        # 进行图像缩放
        h, w, c = self.meta['inp_size']  # 需要的图片大小
    ...
    def detect_object(self, im):
        this_inp = self.preprocess(im)  # 图像预处理
        ...
        result = self.postprocess(out[0])  # 图像后处理
...
'''
    ...
    for filename in image_names:
        im = cv2.imread(filename)  # 加载图片
        h,w,_ = im.shape  # 所加载图片的形状
        results = yolo.detect_object(im)  # 使用yolo进行目标检测
        thick = int((h + w) // 300)  # 用于估算在图片中打印的字体和矩形框的厚度
        draw = im.copy()
        h, w, _ = draw.shape
        for i in range(len(results)):
            # 所属种类
            cv2.putText(draw, 
                        str(results[i]['category']), 
                        (int(w * results[i]['x1']), int(h*results[i]['y1'])-12),
                        0, 
                        1e-3*h, 
                        colors[results[i]['label']], 
                        thick//3)
            # 矩形框
            cv2.rectangle(draw,
                          (int(w*results[i]['x1']),int(h*results[i]['y1'])),
                          (int(w*results[i]['x2']),int(h*results[i]['y2'])), 
                          colors[results[i]['label']], 
                          thick)
